...
//! `Extension::decode` (via [`Extensions::from_sequence`]) to return
        ...
        read_sequence(r, |inner| {
            ...
            let (critical, extn_value_bytes) = if !inner.is_empty() {
                ...
                if any.tag.is_universal(Tag::BOOLEAN) {
                    ...
                    (b, OctetString::decode_from_any(&value_any)?.0)
                } else {
                    (false, OctetString::decode_from_any(&any)?.0)
                }
            } else {
    ...
    fn decode(r: &mut Reader<'a>) -> Result<Self> {
        read_sequence(r, |inner| {
            let ca = if !inner.is_empty()
                && Any::peek_tag(inner) == Some(Tag::universal(Tag::BOOLEAN))
            {
            ...
            let path_len_constraint =
                if !inner.is_empty() { Some(Integer::decode(inner)?.as_u64().ok_or(Error::Custom("pathLenConstraint too large"))?) } else { None };
...
impl<'a> ExtendedKeyUsage<'a> {
    ...
    /// Returns `true` if `anyExtendedKeyUsage` / `id-kp-serverAuth` is present.
    ...
    fn decode(r: &mut Reader<'a>) -> Result<Self> {
        read_sequence(r, |inner| {
            ...
            while !inner.is_empty() {
                ...
                let (full_name, crl_issuer) =
                    read_sequence(&mut sub, |dp| {
